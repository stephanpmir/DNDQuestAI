      ...
      <div className="max-w-lg w-full mx-4 text-center space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="text-lg text-red-300/70">
            {character.name} the {character.race} {character.class} has fallen.
          </div>
          <div className="text-sm text-muted-foreground italic">
            You lie still on the cold ground, your adventure at an end...
          </div>
        </div>

        {/* Final stats */}
        <div className="bg-red-950/30 border border-red-900/50 rounded-lg p-4 space-y-2">
          <div className="text-xs text-red-400/70 uppercase tracking-wider font-semibold">Final Record</div>
          <div className="grid grid-cols-2 gap-2 text-sm">
            ...
            <div className="text-muted-foreground">Gold Collected</div>
            <div className="font-bold text-amber-400">{character.gold}</div>
            <div className="text-muted-foreground">Turns Survived</div>
        ...
        {topScores.length > 0 && (
          <div className="bg-muted/20 border border-border/30 rounded-lg p-4 space-y-2">
            <div className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Hall of the Fallen</div>
            <div className="space-y-1">
              {topScores.map((score, i) => (
                <div key={score.id} className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-muted-foreground">{score.turns} turns</span>
                </div>
              ))}
            </div>
          </div>
